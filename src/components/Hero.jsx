...
export default function Hero({ profile, highlights = [] }) {
  ...
  return (
    <section className="relative min-h-[64vh] lg:min-h-[70vh] overflow-hidden">
      <div className="absolute -top-32 -right-32 h-96 w-96 rounded-full bg-accent/12 blur-3xl motion-safe:animate-float-slow" />
      <div className="absolute -bottom-32 -left-20 h-72 w-72 rounded-full bg-primary/5 blur-3xl motion-safe:animate-float-slow" />

      <div className="max-w-6xl mx-auto px-3 sm:px-5 lg:px-6 pt-24 pb-2 grid items-center gap-6 lg:gap-8 lg:grid-cols-[0.95fr_1.05fr]">
        <div className="space-y-4 text-center lg:text-left">
          <span className="inline-flex items-center justify-center lg:justify-start text-[11px] uppercase tracking-[0.4em] text-secondary/70">
            Research Portfolio
          </span>

          <h1 className="text-5xl md:text-6xl lg:text-[5rem] xl:text-[5.5rem] font-semibold text-primary leading-[0.95] tracking-tight motion-safe:animate-fade-up">
            {profile.name}
          </h1>

          <p className="text-xl md:text-2xl text-secondary font-semibold motion-safe:animate-fade-up-slow motion-safe:[animation-delay:120ms]">
            {profile.title}
          </p>
          ...
          {quickFacts.length > 0 && (
            <div className="grid gap-3 sm:grid-cols-3 pt-1">
              {quickFacts.map((fact, idx) => (
                <div
                  key={idx}
                  className="rounded-2xl border border-border/70 bg-surface/80 px-4 py-3 text-left shadow-[0_12px_24px_rgba(20,18,16,0.08)]"
                >
          ...
          <div className="flex flex-wrap gap-3 justify-center lg:justify-start pt-1">
            ...
            {profile.contacts.cv && (
              <a
                ...
                className="inline-flex items-center px-5 py-2.5 text-sm bg-primary text-white font-semibold rounded-full hover:bg-primary/90 transition-colors duration-200 cursor-pointer shadow-[0_12px_24px_rgba(20,18,16,0.18)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40 focus-visible:ring-offset-2"
              >
                <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="relative lg:justify-self-end w-full max-w-xl self-stretch">
          <div className="bg-surface/95 border border-border/70 shadow-[0_18px_45px_rgba(20,18,16,0.16)] rounded-3xl p-5 md:p-7 backdrop-blur h-full flex flex-col gap-5">
            <div className="relative mb-5 rounded-2xl border border-dashed border-border/80 bg-[linear-gradient(to_right,rgba(20,18,16,0.08)_1px,transparent_1px),linear-gradient(to_bottom,rgba(20,18,16,0.08)_1px,transparent_1px)] bg-[size:16px_16px] p-4">
            ...
            <div className="mt-auto flex flex-wrap justify-center gap-3">
              {profile.contacts.scholar && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 rounded-full border border-border/80 bg-surface/80 px-5 py-2.5 text-sm font-semibold text-primary hover:bg-surface transition-colors duration-200 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40 focus-visible:ring-offset-2"
                >
                  Scholar
